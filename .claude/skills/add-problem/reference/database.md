...
| `input` | `jsonb` | Array of strings, one per case |
...
insert into public.problems
  (id, name, content, input, output, is_active, time_limit, memory_limit, points, generator_file, created_by)
values (
  'ccc25j3',
...
```sql
select id, name, is_active, points, time_limit, memory_limit,
       jsonb_array_length(input)  as cases,
       jsonb_array_length(output) as outs,
       (select max(octet_length(x)) from jsonb_array_elements_text(input) x)  as max_in,
       (select max(octet_length(x)) from jsonb_array_elements_text(output) x) as max_out,
       (select coalesce(sum(octet_length(x)),0) from jsonb_array_elements_text(input) x)
     + (select coalesce(sum(octet_length(x)),0) from jsonb_array_elements_text(output) x) as total_bytes,
       length(content)        as content_len,
       length(generator_file) as generator_len
from public.problems
where id = 'ccc25j3';
```

`cases` must equal `outs`, `max_in` and `max_out` must both be under 1,000,000, and
...
query result — test data runs to hundreds of kilobytes and there is no reason to read it. `problems`
is world-readable under RLS, so fetch it straight to disk with the publishable key:

```bash
KEY=$(sed -n 's/^NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=//p' main/.env.local | tr -d '[:space:]')
URL=$(sed -n 's/^NEXT_PUBLIC_SUPABASE_URL=//p'              main/.env.local | tr -d '[:space:]')

curl -s "$URL/rest/v1/problems?id=eq.ccc25j3&select=input,output,generator_file" \
...
This is read-only and uses the same public key the browser has. It is a convenience for reading;
**writes still go through the Supabase MCP**, never through this endpoint.
