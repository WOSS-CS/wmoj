...
function init() {
    ...
    const particleMat = new THREE.MeshLambertMaterial({ color: 0x4A3020 }); // Slightly darker brown for splatter
...
function shootProjectile() {
    // Poop shape (simple sphere for now)
...
function createSplatter(position, normal, count = 20) {
    for (let i = 0; i < count; i++) {
        ...
        const velocity = randomDir.multiplyScalar(Math.random() * 15 + 5);
        
        particles.push({
            position: position.clone(),
            velocity: velocity,
            life: 0.5 + Math.random() * 0.5 // 0.5 to 1.0 seconds life
        });
    }
}
...
function animate() {
    ...
    if ( controls.isLocked === true ) {
        ...
        for(let i = projectiles.length - 1; i >= 0; i--) {
            ...
            for(let j = targets.length - 1; j >= 0; j--) {
                ...
                if (t.box.intersectsSphere(pSphere)) {
                    // Hit target!
                    createSplatter(p.mesh.position, null, 30);
            ...
            if (!projectileRemoved) {
                ...
                if (p.mesh.position.y <= 0.6) {
                    createSplatter(new THREE.Vector3(p.mesh.position.x, 0.6, p.mesh.position.z), new THREE.Vector3(0, 1, 0), 20);
                    projectileRemoved = true;
                } else if (p.mesh.position.x >= halfArena) {
                    createSplatter(new THREE.Vector3(halfArena, p.mesh.position.y, p.mesh.position.z), new THREE.Vector3(-1, 0, 0), 20);
                    projectileRemoved = true;
                } else if (p.mesh.position.x <= -halfArena) {
                    createSplatter(new THREE.Vector3(-halfArena, p.mesh.position.y, p.mesh.position.z), new THREE.Vector3(1, 0, 0), 20);
                    projectileRemoved = true;
                } else if (p.mesh.position.z >= halfArena) {
                    createSplatter(new THREE.Vector3(p.mesh.position.x, p.mesh.position.y, halfArena), new THREE.Vector3(0, 0, -1), 20);
                    projectileRemoved = true;
                } else if (p.mesh.position.z <= -halfArena) {
                    createSplatter(new THREE.Vector3(p.mesh.position.x, p.mesh.position.y, -halfArena), new THREE.Vector3(0, 0, 1), 20);
                    projectileRemoved = true;
        ...
        if (activeCount > 0 || particleMesh.count === 0) { // always update if 0 so old instances disappear
