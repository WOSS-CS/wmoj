...
export default function ProblemPage() {
  ...
  const problemId = Array.isArray(routeParams?.id)
    ? routeParams.id[0]
    : routeParams?.id;
  ...
  // Use cached resource to avoid refetching the full problem unless it changed
  const problemStore = useCachedResource<Problem>({
    key: `problem:${problemId}`,
    metaUrl: `/api/problems/${problemId}/meta`,
    fetchFull: async () => {
  ...
  useEffect(() => {
    // If cached store exists, set from cache immediately
    if (problemStore && problemStore.data) {
      setProblem(problemStore.data);
      setLoading(false);
    } else if (problemId && session) {
      // fallback if no cache: still fetch
  ...
  useEffect(() => {
    (async () => {
      ...
      if (problem.contest) {
        try {
          const hasAccess = await checkContestParticipation(
            user.id,
            problem.contest
          );
          if (!hasAccess) {
  ...
  useEffect(() => {
    ...
    const countdownResolved =
      isActive !== undefined && (contestId !== null || !isActive);
      ...
      <RegularOnlyGuard>
        <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black relative overflow-hidden">
          ...
          <div className="absolute inset-0 pointer-events-none">
            ...
            {/* Floating particles */}
            <div
              className="absolute top-20 left-20 w-2 h-2 bg-green-400 rounded-full animate-ping"
              style={{ animationDelay: "0s" }}
            ></div>
            <div
              className="absolute top-40 right-32 w-1 h-1 bg-green-400 rounded-full animate-ping"
              style={{ animationDelay: "1s" }}
            ></div>
            <div
              className="absolute bottom-32 left-1/3 w-1.5 h-1.5 bg-green-400 rounded-full animate-ping"
              style={{ animationDelay: "2s" }}
            ></div>
            <div
              className="absolute top-1/2 right-20 w-1 h-1 bg-green-400 rounded-full animate-ping"
            ...
            <div className="absolute inset-0 opacity-10">
              ...
              <div
                className="absolute top-20 left-52 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "0.5s" }}
              ></div>
              <div
                className="absolute top-20 left-52 w-0.5 h-16 bg-gradient-to-b from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "0.5s" }}
              ></div>
              <div
                className="absolute top-36 left-52 w-24 h-0.5 bg-gradient-to-r from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "1s" }}
              ></div>
              <div
                className="absolute top-36 left-76 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "1s" }}
              ></div>

              <div
                className="absolute top-40 right-20 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "1.5s" }}
              ></div>
              <div
                className="absolute top-40 right-20 w-0.5 h-20 bg-gradient-to-b from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "1.5s" }}
              ></div>
              <div
                className="absolute top-60 right-20 w-40 h-0.5 bg-gradient-to-r from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "2s" }}
              ></div>
              <div
                className="absolute top-60 right-60 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "2s" }}
              ></div>

              <div
                className="absolute bottom-32 left-32 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "2.5s" }}
              ></div>
              <div
                className="absolute bottom-32 left-32 w-0.5 h-24 bg-gradient-to-b from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "2.5s" }}
              ></div>
              <div
                className="absolute bottom-8 left-32 w-28 h-0.5 bg-gradient-to-r from-green-400 to-transparent animate-pulse"
                style={{ animationDelay: "3s" }}
              ></div>
              <div
                className="absolute bottom-8 left-60 w-2 h-2 bg-green-400 rounded-full animate-pulse"
                style={{ animationDelay: "3s" }}
              ></div>
            </div>
          </div>
                                    ...
                                    {!r.passed && (
                                      <div className="mt-3 grid md:grid-cols-2 gap-3 text-sm">
                                        <div>
                                          ...
                                          <pre className="p-3 rounded bg-black/40 text-gray-200 whitespace-pre-wrap">
                                            {r.expected}
                                          </pre>
                                        </div>
                                        <div>
