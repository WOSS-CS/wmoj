    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmissions((json.submissions || []).map((s: {
          id: string; created_at: string; problem: string;
      ...
      <AdminGuard>
        <div className="w-full space-y-6">
          <div>
            ...
            <p className="text-sm text-text-muted mt-1 ml-8">View all code submissions for <span className="font-semibold text-foreground">{targetUsername}</span>.</p>
          </div>
            ...
            <div className="w-full max-w-4xl bg-surface-1 border border-border rounded-lg flex flex-col max-h-[90vh]">
              <div className="flex items-center justify-between px-5 py-3 border-b border-border">
                <div>
                  ...
                  <p className="text-xs text-text-muted">by {targetUsername} • {selectedSubmission.problem} • {formatDate(selectedSubmission.timestamp)}</p>
                </div>
