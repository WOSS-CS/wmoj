...
export default function ProblemSubmissionsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchSubmissions = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/admin/problems/${id}/submissions`, { headers: { Authorization: `Bearer ${token}` } });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Failed to load submissions');
            setSubmissions(data.submissions || []);
            if (data.problem_name) setProblemName(data.problem_name);
        } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error fetching submissions'); }
