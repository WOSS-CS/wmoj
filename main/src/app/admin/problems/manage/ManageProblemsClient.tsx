...
import { staffStatusLabel, type StaffStatusFilter } from '@/lib/pagination';
import type { ProblemManageRow } from '@/lib/queries/problems';

/** {@link PROBLEM_MANAGE_COLUMNS} plus the contest names the page enriches with. */
type ProblemRow = ProblemManageRow & { contest_names: string[] };
...
export default function ManageProblemsClient({
  ...
  const columns: Array<DataTableColumn<Row>> = [
    ...
    { key: 'updated', header: 'Updated', className: 'w-2/12', sortable: true, sortAccessor: (r) => (r.updated_at ? new Date(r.updated_at).getTime() : 0), render: (r) => <span className="text-text-muted text-sm font-mono">{r.updated_at ? new Date(r.updated_at).toLocaleDateString() : '—'}</span> },
    {
      ...
      <div className="w-full space-y-6">
        ...
        <div className="glass-panel overflow-hidden">
          <div className="bg-surface-2 px-4 py-3 border-b border-border">
            <h2 className="text-sm font-semibold text-foreground">{staffStatusLabel(currentStatus)} Problems</h2>
          </div>
