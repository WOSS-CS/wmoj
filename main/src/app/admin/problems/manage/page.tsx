...
export default function ManageProblemsPage() {
  ...
  const fetchContests = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) setAvailableContests(data.contests || []);
  ...
  const fetchProblems = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/admin/problems/list', { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load');
      setProblems(data.problems || []);
    } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error fetching problems'); }
    finally { setLoading(false); }
  }, [token]);

  useEffect(() => { if (token) { setError(null); fetchProblems(); fetchContests(); } }, [token, fetchProblems, fetchContests]);
