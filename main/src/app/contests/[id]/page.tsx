...
export default function ContestPage() {
  ...
  // Use cached resource hook to avoid unnecessary full reloads when data hasn't changed
  const contestStore = useCachedResource<{
    contest: Contest;
    problems: { id: string; name: string }[];
    leaderboard: any[];
  }>({
    key: `contest:${params.id}`,
    metaUrl: `/api/contests/${params.id}/meta`,
    fetchFull: async () => {
      const res = await fetch(`/api/contests/${params.id}`);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || "Failed to load contest");
      // If system has access token, include it
      ...
      return {
        ...
        problems: jsonP.problems || [],
        leaderboard: jsonL.leaderboard || [],
      };
    },
    intervalMs: 15000,
  ...
  useEffect(() => {
    // Update local state from cache store as soon as it is available
    if (!contestStore) return;
    if (contestStore.data) {
      setContest(contestStore.data.contest);
      setProblems(contestStore.data.problems || []);
      setLeaderboard(contestStore.data.leaderboard || []);
      setLoading(false);
      setIsLoaded(true);
    }
  }, [contestStore]);
              ...
              >
                ...
                <div className="flex justify-between items-start mb-8">
                  ...
                  <div className="flex gap-3 ml-6">
                    <button
                      ...
                      className="px-6 py-3 border border-green-400/50 text-green-400 rounded-lg hover:bg-green-400/10 transition-all duration-300 relative overflow-hidden group/btn"
                    >
                    ...
                    <button
                      ...
                      disabled={leaving}
                      className="px-6 py-3 border border-white/30 text-gray-300 rounded-lg hover:border-white/50 hover:text-white hover:bg-white/5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed relative overflow-hidden group/btn"
                    >
                      <span className="relative z-10 flex items-center gap-2">
                        {leaving ? (
                          <>
                          ...
                          >
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-gray-400 text-sm flex items-center gap-2">
                                ...
                                {entry.solved_problems}/{entry.total_problems}{" "}
                                solved
                              </div>
                ...
                {problems.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-400">
                      Problems will appear here when they&apos;re added to the
                      contest.
                    </p>
                  </div>
                ) : (
                    ...
                    {problems.map((p, index) => (
                      <Link
                        ...
                        className={`bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20 hover:border-green-400/50 transition-colors duration-300 group ${
                          hoveredProblem === p.id
                            ? "scale-105 shadow-lg shadow-green-400/20 border-green-400/50"
                            : ""
                        }`}
                      ...
                      >
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          ...
                          <svg
                            className="w-5 h-5 text-green-400 transition-colors duration-300"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
