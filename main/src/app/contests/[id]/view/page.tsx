...
export default function ContestViewPage() {
  ...
  const contestStore = useCachedResource<ContestDetail>({
    ...
    fetchFull: async () => {
      ...
      return json.contest as ContestDetail;
    },
    enabled: !!params?.id,
    intervalMs: 15000,
  });
  ...
  useEffect(() => {
    if (contestStore && contestStore.data) {
      setContest(contestStore.data);
      setLoading(false);
    }
  }, [contestStore]);
        ...
        <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black relative overflow-hidden">
          ...
          <nav className="relative z-10 flex justify-between items-center p-4 backdrop-blur-sm border-b border-white/10">
            ...
            <div className="flex items-center gap-4">
              <Link
                href="/contests"
                className="px-4 py-2 text-white border border-white/20 rounded-lg hover:bg-white/10 transition-all duration-300"
              >
                Back
              </Link>
