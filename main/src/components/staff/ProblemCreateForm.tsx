...
/**
 * The Create Problem form, shared by both staff trees.
 *
 * `app/admin/problems/create/CreateProblemClient.tsx` and its manager twin were
 * 203 lines each differing in ONE line — the export name — plus the four URLs
 * and the guard wrapper, all of which are derived from `tree` here. The two
 * clients are adapters now, so the form cannot drift between the trees again.
 *
 * Nothing about creating a problem differs by tree: both trees post an
 * identical payload and the server decides whether the row lands pending
 * (`STAFF_POLICY.<tree>.createsPending`). If that ever stops being true, the
 * flag belongs in `lib/staffPolicy.ts`, not in a branch here.
 */
export function ProblemCreateForm({ tree }: ProblemCreateFormProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(routes.generate, { method: 'POST', headers, body: JSON.stringify({ code: generatorCode }) });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res = await fetch(`${routes.api}/problems/create`, {
        method: 'POST', headers,
  ...
  return (
    <AuthGuard requireAuth allowAuthenticated>
      <div className="w-full space-y-6">
        ...
        <ProblemCreationGuideModal open={showGuide} onClose={() => setShowGuide(false)} role={tree} />
      </div>
    </AuthGuard>
  );
}
