...
import { STAFF_POLICY, staffRoutes, type StaffTree } from '@/lib/staffPolicy';
...
export interface ContestCreateFormProps {
  tree: StaffTree;
}

/**
 * The Create Contest form, shared by both staff trees.
 *
 * The two clients this replaces were 204/203 lines apart by five: the URLs, a
 * "Summary Preview" marker comment the manager copy had lost, and the Active
 * preview cell, which was hard-coded `No` on the admin side and `Yes` on the
 * manager side. That cell is the one genuine difference and it is now read from
 * `STAFF_POLICY[tree].createsPending` rather than typed out twice.
 *
 * The payload carries no `is_active` key on either tree — the server decides —
 * so the preview is a MIRROR of the route's behaviour, not an instruction to
 * it. If the two ever disagree, the route is right and this cell is wrong.
 */
export function ContestCreateForm({ tree }: ContestCreateFormProps) {
  const routes = staffRoutes(tree);
  // Admin creations land pending; manager creations go live immediately.
  const landsActive = !STAFF_POLICY[tree].createsPending;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${routes.api}/contests/create`, {
        method: 'POST',
      ...
      <div className="w-full space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
          ...
          <div className="space-y-1.5">
            ...
            <ProblemSearch
              searchEndpoint={routes.problemSearch}
          ...
          <div>
            {(() => {
              type Row = { name: string; description: string; length: number };
              const columns: Array<DataTableColumn<Row>> = [
                ...
                { key: 'active', header: 'Active', className: 'w-[10%]', render: () => <span className={`text-xs font-medium ${landsActive ? 'text-success' : 'text-warning'}`}>{landsActive ? 'Yes' : 'No'}</span> },
              ];
              return <DataTable<Row> columns={columns} rows={[{ name: formData.name, description: formData.description, length: formData.length }]} rowKey={(_r, i) => String(i)} />;
            })()}
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Link href={routes.dashboard} className="h-10 px-5 bg-surface-2 text-foreground text-sm font-medium rounded-md hover:bg-surface-3 flex items-center">Cancel</Link>
          </div>
        </form>
      </div>
    </AuthGuard>
