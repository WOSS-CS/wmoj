...
import type { ContestEditRow } from '@/lib/queries/contests';
...
export interface ContestEditFormProps {
  tree: StaffTree;
  contest: ContestEditRow;
  initialProblems?: SearchableProblem[];
}

/**
 * The Edit Contest form, shared by both staff trees.
 *
 * The two clients this replaces were 216 lines each differing in ONE line — the
 * export name — plus the three URLs and the guard wrapper, all derived from
 * `tree` here.
 *
 * The form itself is tree-invariant on purpose. `STAFF_POLICY.admin`'s
 * activated-contest guard ("only managers edit an already-activated contest")
 * is enforced by the PATCH route, not here: the page is reachable for both
 * trees and a blocked save comes back as an error in the banner below. Do not
 * re-implement that guard client-side — a second copy of a rule the server owns
 * is exactly what this component exists to prevent.
 *
 * `is_active` is displayed but never submitted; only a manager may flip it, and
 * only through the route.
 */
export function ContestEditForm({ tree, contest, initialProblems = [] }: ContestEditFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${routes.api}/contests/${contest.id}`, {
        method: 'PATCH',
      ...
      if (res.ok && json.contest) {
        ...
        setTimeout(() => router.push(routes.contestsManage), 2000);
      ...
      <div className="w-full space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 max-w-4xl">
          ...
          <div className="flex gap-3">
            ...
            <Link href={routes.contestsManage} className="h-10 px-5 bg-surface-2 text-foreground text-sm font-medium rounded-md hover:bg-surface-3 flex items-center">Cancel</Link>
          </div>
        </form>
      </div>
    </AuthGuard>
